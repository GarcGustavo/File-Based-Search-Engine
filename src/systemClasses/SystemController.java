...
public class SystemController {
										 ...
										 RandomAccessFile docFile) 
	{
		...
		for (WordInDocument wid : document) {
			ArrayList<Integer> wordDocsList = documentWordsMap.get(wid.getWord());	
			// ADD MISSING CODE HERE (Exercise 4)
			if(wordDocsList == null)
				wordDocsList = new ArrayList<Integer>();
			wordDocsList.add((int) wid.getLocation());
			documentWordsMap.put(wid.getWord(), wordDocsList);
